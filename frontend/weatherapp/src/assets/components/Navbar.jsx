...
const Navbar = ({search_val,search_fn,weather_fn}) => {
  ...
  return (
    <header className='sticky top-0 '>
      <nav className=' flex bg-black justify-between items-center py-5 md:px-8 pl-5 pr-2'>
        <div className='logo font-bold text-[20px]'>WeatherX</div>
        <div className=' relative'>
          <img className=' left-1 top-1.5 absolute w-6 h-6 ' src='./search.svg' />
          <input onChange={handleSet} value={search_val} onKeyDown={(e) => {
            if (e.key === "Enter")
              handleSearch();
          }} className=' bg-[#0e0d0d] rounded-full md:pl-8 md:pr-4 pl-7 pr-2 py-1.5 outline-none border-none' type='search' placeholder='Search City' />
        </div>
      </nav>
    </header>
  )
